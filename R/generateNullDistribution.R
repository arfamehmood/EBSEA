...
generateNullDistribution <- function(x)
{
    ...
    for(i in 1:length(exonCounts))
    {
        nullDistribution <- NULL
        for(j in 1:exonCounts[i])
        {
            nullDistribution <- cbind(nullDistribution, sample(c(-1, 1),
            N, replace=TRUE) * (-log(runif(N, min=0, max=1))))
        }
        M <- apply(nullDistribution, 1, medianP)
        ind <- which(x$ExonCount == exonCounts[i])
        med <- x$Median[ind]
